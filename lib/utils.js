...
module.exports = {
    ...
    shouldBypass: (node, context, rawTests, filename) => {
        ...
        const ignoreFileName = typeof context.settings.ignoreFileName === 'string' && context.settings.ignoreFileName;
        if (ignoreFileName) {
            const paths = filename.split('/');
            const fileWithExtension = paths[paths.length - 1];
            return ignoreFileName === fileWithExtension;
        }
